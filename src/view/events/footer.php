<footer>
  <div class="footer">
    <div>
      <h2 class="footer-title"><span class="hidden">Spekken</span></h2>
      <address>
        <strong>Tinnenpot</strong> <br /> Tinnenpotstraat 21 <br /> 9000 Gent
      </address>
    </div>
    <section class="footer-fb">
      <h2>Sociale media</h2>
      <p><a class="fbLink" href="https://www.facebook.com/Spekken/" target="_blank">Facebook</a></p>
    </section>
    <section class="footer-nav">
      <h2 class="hidden">Navigatie</h2>
      <article class="">
        <h2 class="footer-nav-title">Agenda</h2>
        <ul>
          <li><a class="footer-nav-item" href="#">Kerstvakantie</a></li>
          <li><a class="footer-nav-item" href="#">Paasvakantie</a></li>
        </ul>
      </article>
      <article class="">
        <h2 class="footer-nav-title">Praktisch</h2>
        <ul>
          <li><a class="footer-nav-item" href="#">Wat is Spekken</a></li>
          <li><a class="footer-nav-item" href="#">Tinnenpot</a></li>
          <li><a class="footer-nav-item" href="#">Tickets</a></li>
        </ul>
      </article>
      <article class="">
        <h2 class="footer-nav-title">Nieuws</h2>
        <ul>
          <li><a class="footer-nav-item" href="#">Blog</a></li>
          <li><a class="footer-nav-item" href="#">Acties</a></li>
        </ul>
      </article>
    </section>
  </div>
</footer>
